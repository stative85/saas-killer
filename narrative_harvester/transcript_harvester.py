...
def vtt_to_clean_text(vtt_path):
    ...
    for line in raw.splitlines():
        ...
        if re.match(r"^\d+$", s):
            continue
        # strip inline tags like <c>, </c>, <00:01:23.456>
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Transcript Harvester")
    ...
    def _build_ui(self):
        ...
        ttk.Label(self, text="Transcript Harvester", style="H.TLabel").pack(pady=(12, 2))
        ...
        self.log = tk.Text(lf, bg="#16161e", fg="#9aa5ce", font=("Consolas", 9),
    ...
    def _set_running(self, v):
        self._running = v
        self.go_btn.configure(state="disabled" if v else "normal")
    ...
    def _do_run(self, url, out_root):
        ...
        try:
            ...
            self.after(0, self._print, f"yt-dlp error: {e}", "err")
            return
        ...
        # Convert each VTT → TXT
        txt_paths = []
        for vtt in vtts:
            if not self._running:
                return
            base = os.path.splitext(os.path.basename(vtt))[0]
            # Strip .en / .en-US language suffix
            ...
            try:
                ...
                self.after(0, self._print, f"  ERR: {os.path.basename(vtt)} - {e}", "err")
        ...
        if self.merge_var.get() and len(txt_paths) > 1:
            ...
            try:
                ...
                self.after(0, self._print, f"Merge error: {e}", "err")
