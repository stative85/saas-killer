...
class NarrativeCompiler:
    ...
    def _load_feedback_bias(self):
        ...
        if best_dir.exists():
            for f in best_dir.glob("*.txt"):
                pos.update(re.findall(r"\b\w{6,}\b", f.read_text(encoding="utf-8").lower()))
    ...
    def select_hook(self, mode="fracture") -> ScriptPhase:
        if mode == "retention_aggressive":
            # Shortest, highest quality, highest resonance
    ...
    def build_narrative(self, count=2, mode="fracture") -> List[ScriptPhase]:
        ...
        for i in range(min(count, len(selected_arcs))):
            ...
            if mode == "retention_aggressive":
                # Prefer shorter, direct sentences
    ...
    def compile(self, mode="fracture", style="aggressive") -> Tuple[str, Dict[str, Any]]:
        ...
        for p in phases:
            content = p.content
            if mode == "retention_aggressive":
                # Tighten: Remove excessive ellipses or breaks if they were in the text
...
def main():
    ...
    ap.add_argument("--mode", choices=["fracture", "closed", "retention_aggressive"], default="fracture")
